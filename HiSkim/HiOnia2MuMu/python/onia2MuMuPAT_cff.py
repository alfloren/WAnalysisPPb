...
def onia2MuMuPAT(process, GlobalTag, MC=False, HLT='HLT', Filter=True):
    ...
    IN_ACCEPTANCE = '(abs(eta) <= 2.4 && pt > 10 )'
    ...
    process.onia2MuMuPatTrkTrk = cms.EDProducer('HiOnia2MuMuPAT',
        ...
        higherPuritySelection = cms.string("(isGlobalMuon || isTrackerMuon || (innerTrack.isNonnull && hiGenParticleRef(0).isNonnull)) && abs(innerTrack.dxy)<4 && abs(innerTrack.dz)<35"),
        # BOTH muons must pass this selection
        lowerPuritySelection  = cms.string("(isGlobalMuon || isTrackerMuon || (innerTrack.isNonnull && hiGenParticleRef(0).isNonnull)) && abs(innerTrack.dxy)<4 && abs(innerTrack.dz)<35"),
    ...
    TRACK_CUTS = "(1)"
    GLB_CUTS = "(isTrackerMuon && isGlobalMuon && globalTrack.normalizedChi2<10 && abs(globalTrack.dxy)<0.02 && numberOfMatches>1 && globalTrack.hitPattern.numberOfValidPixelHits > 1 && globalTrack.hitPattern.numberOfValidTrackerHits >11)"
    ...
    process.tpPairsSta = cms.EDProducer("CandViewShallowCloneCombiner",
                                        cut = cms.string('40.0 < mass < 140.0'),
